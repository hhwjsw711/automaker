...
import { isAuthenticatedFn, isAdminFn } from "~/fn/auth";
import {
  Card,
  CardContent,
  CardDescription,
...
const getLaunchKitQuery = (slug: string) =>
  queryOptions({
    queryKey: ["launch-kit", slug],
    queryFn: () => getLaunchKitBySlugFn({ data: { slug } }),
  });

const getCommentsQuery = (slug: string) =>
  queryOptions({
                  ...
                  {isAdmin && (
                    <Button variant="ghost" size="sm" asChild>
                      <Link
                        ...
                        params={{ id: launchKit.id.toString() }}
            ...
            <div className="space-y-4">
              {commentsLoading ? (
                ...
                comments?.map((comment: any) => (
                  <div
                    key={comment.id}
                    className="flex gap-3 p-4 rounded-lg bg-muted/20"
                  >
                    <Avatar>
